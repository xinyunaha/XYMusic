...
import github.xinyunaha.music_hd.moudles.MyLayoutManager
...
//        }
        ...
        request.hotSearch().enqueue(object : Callback<hotSearch> {
            ...
            override fun onResponse(call: Call<hotSearch>, response: Response<hotSearch>) {
                ...
                HotSearchList.layoutManager = layout
                ...
                request.search("$query").enqueue(object : Callback<search> {
                    ...
                    override fun onResponse(call: Call<search>, response: Response<search>) {
                        ...
                        for (i in response.body()!!.result.songs.indices) {
                            ...
                            d(
                                "结果输出",
                                "Number=$i,ID=${songs.id.toLong()},Name=${songs.name},Singer=${songs.artists[0].name},Album=${songs.album.name},Time=${
                                    formatTime(
                                        songs.duration
                                    )
                                }"
